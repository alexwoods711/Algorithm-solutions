...
class CodeTask:
    ...
    def __init__(
        ...
        def process_example(x):
            ...
            inputs = [
                json.dumps(tests["inputs"][i]) for i in range(len(tests["inputs"]))
            ]
            outputs = [
                json.dumps(tests["outputs"][i]) for i in range(len(tests["outputs"]))
            ]
            ...
            encoded_tests = {"inputs": inputs, "outputs": outputs, "fn_name": fn_name}
    ...
    async def evaluate(
        self, response: str, challenge: Challenge, timeout: int = DEFAULT_TEST_TIMEOUT
    ) -> tuple[float, str]:
        """
    ...
    async def _run_test_subprocess(
        self, code: str, timeout: int, test_index: int, stdin_input: str = None
        ...
        try:
            ...
            monitor_task = asyncio.create_task(
                self._monitor_process_memory(process, test_index)
            )
    ...
    async def _cleanup_process(self, process, test_index: int):
        ...
        try:
            ...
            subprocess.run(
                ["pkill", "-9", "-P", str(process.pid)], capture_output=True, timeout=1
            )
        except:
            pass
    ...
    async def _run_stdin_test(
        ...
        try:
            ...
            print(
                ...
                repr(actual),
                repr(expected),
                compare_stdout_results(actual, expected),
                stderr,
            )
    ...
    async def _run_function_test(
        ...
        try:
            ...
            wrapper_code = generate_function_wrapper(
                f"{BASE_IMPORTS}\n{code}", fn_name, test_input_str
            )

            process, stdout, stderr = await self._run_test_subprocess(
                wrapper_code.replace(
                    f"{BASE_IMPORTS}\n", ""
            ...
            if exec_outputs == test_case_outputs:
                return True
            if (
                isinstance(test_case_outputs, list)
                and exec_outputs == test_case_outputs[0]
            ):
                return True
